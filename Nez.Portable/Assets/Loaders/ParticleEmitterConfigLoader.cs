...
namespace Nez.ParticleDesigner
{
    public static class ParticleEmitterConfigLoader
    {
        ...
        private static ParticleEmitterConfig Load(XDocument xDoc, string rootDir)
        {
            ...
            {
                string path = Path.Combine(rootDir, (string)textureElement.Attribute("name"));
        ...
        private static float GetFloatElement(XElement root, string name) => (float)root.Element(name).Attribute("value");

        /// <summary>
        /// for some reason, some pex exporters export ints like maxParticles as floats. This mess guards against that.
        /// </summary>
        private static int GetIntElement(XElement root, string name) => Mathf.RoundToInt(GetFloatElement(root, name));

        private static Vector2 GetVectorElement(XElement root, string name)
        {
            var ele = root.Element(name);
            return new Vector2((float)ele.Attribute("x"), (float)ele.Attribute("y"));
        }

        private static Color GetColorElement(XElement root, string name)
        {
            var ele = root.Element(name);
            return new Color((float)ele.Attribute("red"), (float)ele.Attribute("green"), (float)ele.Attribute("blue"), (float)ele.Attribute("alpha"));
        }

        private static Blend GetBlendElement(XElement root, string name)
        {
            int value = GetIntElement(root, name);
            switch (value)
            {
                case 0:
                    return Blend.Zero;
